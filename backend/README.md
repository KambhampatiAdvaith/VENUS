...
- `ENABLE_KAFKA_TELEMETRY_CONSUMER=false` to start the real Kafka telemetry consumer inside FastAPI and broadcast telemetry WebSocket events after database writes
- `ENABLE_KAFKA_FAULT_CONSUMER=false` to start the real Kafka fault consumer inside FastAPI and broadcast fault WebSocket events after database writes
